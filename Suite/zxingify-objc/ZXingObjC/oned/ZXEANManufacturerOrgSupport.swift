 ...
 */
@objc
class ZXEANManufacturerOrgSupport: NSObject {
    private var _countryIdentifiers: NSMutableArray!
    private var _ranges: NSMutableArray!

    @objc
    override init() {
        if self = super.init() {
            _ranges = NSMutableArray()
            _countryIdentifiers = NSMutableArray()
        }
    ...
    @objc
    func lookupCountryIdentifier(_ productCode: String!) -> String? {
        self.initIfNeeded()

        let prefix: CInt = productCode.substringToIndex(3).intValue()
        let max: UInt = UInt(UInt(self.ranges.count ?? 0))
        var i: CInt = 0
        ...
        while i < max {
            ...
            let range: NSArray! = self.ranges[Int(i)]
            let start: CInt = range?[0].intValue()

            if prefix < start {
                return nil
            }

            let end: CInt = (range?.count == 1) ? start : range?[1].intValue()

            if prefix <= end {
                return self.countryIdentifiers[Int(i)]
            }
        }
    ...
    @objc
    func add(_ range: NSArray!, identifier: String!) {
        self.ranges.add(range)
        self.countryIdentifiers.add(identifier)
    }
    @objc
    func initIfNeeded() {
        let _lockTarget = self.ranges
 ...
 */
@objc
extension ZXEANManufacturerOrgSupport {
    @objc var countryIdentifiers: NSMutableArray! {
        return self._countryIdentifiers
    }
    @objc var ranges: NSMutableArray! {
        return self._ranges
    }
}