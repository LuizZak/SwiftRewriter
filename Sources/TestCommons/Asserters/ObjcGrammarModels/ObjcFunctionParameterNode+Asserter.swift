...
public extension Asserter where Object == ObjcFunctionParameterNode {
    /// Asserts that the underlying `ObjcFunctionParameterNode` being tested has
    ...
    /// Asserts that the underlying `ObjcFunctionParameterNode` being tested has
    /// no identifier node associated with it.
    ///
    /// Returns `nil` if the test failed, otherwise returns `self` for chaining
    /// further tests.
    @discardableResult
    ...
    /// Asserts that the underlying `ObjcFunctionParameterNode` being tested has
    /// a type node with a specified `type` value.
    ///
    /// Returns `nil` if the test failed, otherwise returns `self` for chaining
    /// further tests.
    @discardableResult
