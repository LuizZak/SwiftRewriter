...
class FunctionSignatureParserTests: XCTestCase {
    ...
    func testFunctionSignatureWithParameters() {
        ...
        assert(
            string: "function(_ int: Int) -> Double",
            parseInto: FunctionSignature(
                name: "function",
                parameters: [
                    ParameterSignature(label: nil, name: "int", type: .int)
    ...
    func testParseFullSignature() {
        assert(
            string:
                "dateByAddingUnit(_ component: Calendar.Component, value: Int, toDate date: Date, options: NSCalendarOptions) -> Date?",
            parseInto: FunctionSignature(
    ...
    func testUnexpectedCharacterInArgumentList() {
        do {
            ...
            XCTAssertEqual(
                lexError.description(withOffsetsIn: "(=)"),
    ...
    func testInvalidDefaultArgument() {
        do {
            ...
            XCTAssertEqual(
                lexError.description(withOffsetsIn: "(a: Int = a)"),
