...
class ExpressionTests: XCTestCase {
    ...
    func testDescriptionExpressions() {
        ...
        XCTAssertEqual(
            Expression.identifier("abc").call([
                .labeled("label", .constant(1)), .unlabeled(.constant(true)),
            ]).description,
            "abc(label: 1, true)"
        )
        XCTAssertEqual(
    ...
    func testDescriptionNullCoalesce() {
        XCTAssertEqual(
            Expression.binary(lhs: .identifier("abc"), op: .nullCoalesce, rhs: .identifier("def"))
                .description,
            "abc ?? def"
        )
    }
    ...
    func testUnknownEquality() {
        XCTAssertEqual(
            Expression.unknown(UnknownASTContext(context: "")),
    ...
    func testUnwrappingParens() {
        XCTAssertEqual(
            Expression.constant(1).unwrappingParens,
            Expression.constant(1)
        )

        XCTAssertEqual(
            Expression.parens(Expression.constant(1)).unwrappingParens,
            Expression.constant(1)
        )

        XCTAssertEqual(
            Expression.parens(Expression.parens(Expression.constant(1))).unwrappingParens,
            Expression.constant(1)
        )
    }
    ...
    func testLiteralExpressionKindIsNilForAllNonLiteralExpressions() {
        ...
        XCTAssertNil(
            Expression.assignment(lhs: .constant(0), op: .assign, rhs: .constant(0))
                .literalExpressionKind
        )
        XCTAssertNil(Expression.cast(.constant(0), type: .int).literalExpressionKind)
        XCTAssertNil(
            Expression.binary(lhs: .constant(0), op: .assign, rhs: .constant(0))
                .literalExpressionKind
        )
