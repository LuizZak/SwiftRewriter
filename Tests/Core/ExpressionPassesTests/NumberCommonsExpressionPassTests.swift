...
class NumberCommonsExpressionPassTests: ExpressionPassTestCase {
    ...
    func testConvertVariableDeclarations() {
        assertTransform(
            // a = b
            statement: .variableDeclaration(identifier: "a", type: .int, initialization: .identifier("b").typed(.float).typed(expected: .int)),
            // a = Int(b)
            into: .variableDeclaration(identifier: "a", type: .int, initialization: .identifier("Int").call([.identifier("b")]))
        ); assertNotifiedChange()
    }
