...
class ObjcLexer_TokenizerTests: XCTestCase {
    ...
    func testTokenizeSequence() {
        expect(
            sequence: "const __weak identifier",
            toTokenizeAs: [
                .typeQualifier("const"), .typeQualifier("__weak"), .identifier("identifier"),
            ]
        )
    }
}
...
extension ObjcLexer_TokenizerTests {
    ...
    private func expect(
        ...
        toTokenizeAs expectedTypes: [ObjcTokenType],
    ...
    private func expect(
        _ string: String,
        toTokenizeAs expectedType: ObjcTokenType,
    ...
    private func tokenizeTest(
        _ string: String,
        _ expected: ObjcLexer.Token,
        file: StaticString = #filePath,
        line: UInt = #line
    ) {
        ...
        if lexer.token() != expected {
            XCTFail(
                "Expected token: \(expected) received token: \(lexer.token())",
                file: file,
                line: line
            )
        }
