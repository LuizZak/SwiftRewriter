...
class ObjcParser_ObjcTypeTests: XCTestCase {
    ...
    func testParseTypeSpecifiers() throws {
        try assertObjcTypeParse(
            "const __weak NSArray<NSString*>* _Nonnull",
            .specified(
                specifiers: ["const", "__weak"],
                .qualified(
                    .pointer(
                        .generic(
                            "NSArray",
        ...
        try assertObjcTypeParse(
            "static __weak NSArray<const NSString*>* _Nonnull",
            .specified(
                specifiers: ["static", "__weak"],
                .qualified(
                    .pointer(
                        .generic(
                            "NSArray",
        ...
        try assertObjcTypeParse(
            "__kindof __weak NSArray*",
            .specified(
                specifiers: ["__kindof", "__weak"],
                .pointer(
                    .struct("NSArray")
                )
            )
    ...
    func testParseGenericObjcType() throws {
        ...
        try assertObjcTypeParse(
            ...
            .pointer(
                .generic(
                    "NSDictionary",
                    parameters: [.pointer(.struct("NSString")), .pointer(.struct("NSNumber"))]
                )
            )
        )
    }
    ...
    func testParseIdWithProtocolListObjcType() throws {
        ...
        try assertObjcTypeParse(
            "id<UITableViewDelegate, UITableViewDataSource>",
            .id(protocols: ["UITableViewDelegate", "UITableViewDataSource"])
        )

        try assertObjcTypeParse(
            "id < UIDelegate , _MyDelegateProtocol >",
            .id(protocols: ["UIDelegate", "_MyDelegateProtocol"])
        )
    }

    private func assertObjcTypeParse(
        _ source: String,
        _ expectedType: ObjcType,
        ...
        do {
            ...
            _ = try sut.withTemporaryContext(
                nodeType: ObjcGlobalContextNode.self,
                do: {
            ...
            if type != expectedType {
                XCTFail(
                    "Failed: Expected to parse type '\(source)' as '\(expectedType)', but received '\(type as Any)'",
                    file: file,
                    line: line
                )
            }
            ...
            if sut.diagnostics.errors.count != 0 {
                XCTFail(
                    "Error(s) parsing type: \(sut.diagnostics.errors.description)",
                    file: file,
                    line: line
                )
            }
