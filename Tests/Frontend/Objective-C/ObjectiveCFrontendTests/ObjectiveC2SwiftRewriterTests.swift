...
class ObjectiveC2SwiftRewriterTests: XCTestCase {
    ...
    func testRewriteSelectorExpression() {
        assertRewrite(
            ...
            swift: """
                class MyClass {
                    func myMethod() {
                        if self.responds(to: #selector(abc(_:))) {
                            thing()
                        }
                    }
                }
    ...
    func
        testBackingFieldAnalysisForSynthesizedPropertyIsIgnoredIfSynthesizedNameMatchesPropertyName()
    {
        assertRewrite(
            objc: """
                @interface A: NSObject
