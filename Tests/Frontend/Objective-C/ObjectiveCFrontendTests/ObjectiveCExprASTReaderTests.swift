...
class ObjectiveCExprASTReaderTests: XCTestCase {
    ...
    func testSelectorExpression() {
        ...
        assert(
            objcExpr: "@selector(abc:def:)",
            readsAs: Expression.selector(
                FunctionIdentifier(name: "abc", argumentLabels: [nil, "def"])
            )
        )
        assert(
            objcExpr: "@selector(abc::def:)",
            readsAs: Expression.selector(
                FunctionIdentifier(name: "abc", argumentLabels: [nil, nil, "def"])
            )
        )
        assert(
            objcExpr: "@selector(abc::def::)",
            readsAs: Expression.selector(
                FunctionIdentifier(name: "abc", argumentLabels: [nil, nil, "def", nil])
            )
        )
    }
    ...
    func testPostfixStructAccessWithAssignment() {
        ...
        assert(
            objcExpr: "self->_ganttEndDate = ganttEndDate",
            readsAs: exp
        )
    }
    ...
    func testBlockExpression() {
        ...
        assert(
            objcExpr: "^NSString*{ return thing(); }",
            readsAs: .block(
                parameters: [],
        ...
        assert(
            objcExpr: "^NSString*(NSInteger inty){ return thing(); }",
            readsAs: .block(
        ...
        assert(
            objcExpr: "^(NSInteger inty){ return thing(); }",
            readsAs: .block(
                parameters: [BlockParameter(name: "inty", type: .int)],
                return: .void,
                body: [
