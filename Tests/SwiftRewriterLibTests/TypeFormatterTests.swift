...
class TypeFormatterTests: XCTestCase {
    ...
    func testAsStringFunctionSignature() {
        ...
        let sig3 = FunctionSignature(name: "abc", parameters: [ParameterSignature(label: "a", name: "b", type: .float), ParameterSignature(label: "c", name: "c", type: .int)], returnType: .void, isStatic: true)
        ...
        XCTAssertEqual("abc(a b: Float, c: Int)", TypeFormatter.asString(signature: sig3, includeName: true))
        XCTAssertEqual("(a b: Float, c: Int)", TypeFormatter.asString(signature: sig3, includeName: false))
        ...
        XCTAssertEqual("(a b: Float, c: Int)", TypeFormatter.asString(signature: sig3))
    }
}
