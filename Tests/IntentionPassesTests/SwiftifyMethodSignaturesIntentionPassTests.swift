            ...
            .method(
                withSignature:
                    FunctionSignature(
                        ...
                        parameters: [
                            ParameterSignature(
                                label: nil, name: "color", type: .typeName("CGColor"))
                        ])
            )
            .converts(to:
                FunctionSignature(
        ...
        testThat(sut: sut)
            .method(withSignature:
                FunctionSignature(
                    ...
                    parameters: [
                        ParameterSignature(
                            label: nil, name: "objects", type: .array(.typeName("Object")))
                    ],
                    returnType: .instancetype,
                    isStatic: false
                )
            ...
            .converts(toInitializer: [
                ParameterSignature(
                    label: "objectList", name: "objects", type: .array(.typeName("Object"))
                )
            ])
    }
...
extension SwiftifyMethodSignaturesIntentionPassTests {
    fileprivate func testThat(
        typeName: String = "T", sut: SwiftifyMethodSignaturesIntentionPass
...
private class SwiftifyMethodSignaturesIntentionPassTestBuilder {
    ...
    init(
        testCase: XCTestCase, typeName: String = "T", sut: SwiftifyMethodSignaturesIntentionPass
    ) {
    ...
    private func createSwiftMethodSignatureGen() -> SwiftMethodSignatureGen {
        let mapper = DefaultTypeMapper(
            typeSystem: IntentionCollectionTypeSystem(intentions: intentions))
    ...
    class Asserter {
        ...
        init(
            testCase: XCTestCase, intentions: IntentionCollection, type: TypeGenerationIntention
        ) {
            self.testCase = testCase
            self.intentions = intentions
            self.type = type
            self.typeMapper = DefaultTypeMapper(
                typeSystem: IntentionCollectionTypeSystem(intentions: intentions))
        }

        func converts(
            toInitializer parameters: [ParameterSignature], file: StaticString = #filePath,
            line: UInt = #line
        ) {
        ...
        func converts(
            toInitializer expected: String, file: StaticString = #filePath, line: UInt = #line
        ) {
            guard let ctor = type.constructors.first else {
                XCTFail(
                    """
        ...
        func converts(
            to signature: FunctionSignature, file: StaticString = #filePath, line: UInt = #line
        ) {
        ...
        func dumpType() -> String {
            ...
            if !type.methods.isEmpty {
                ...
                for method in type.methods {
                    dump +=
                        "Method: \(TypeFormatter.asString(signature: method.signature, includeName: true))"
                    dump += "\n"
                }
            }
